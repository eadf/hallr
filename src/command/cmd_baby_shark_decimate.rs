...
pub(crate) fn process_command(
    ...
    let error_threshold: f32 = config.get_mandatory_parsed_option("ERROR_THRESHOLD", None)?;
    let min_faces_count: usize = config.get_mandatory_parsed_option("MIN_FACES_COUNT", None)?;
    ...
    let decimation_criteria = ConstantErrorDecimationCriteria::new(error_threshold);
    let mut decimator = EdgeDecimator::new()
        .decimation_criteria(decimation_criteria)
        .min_faces_count(Some(min_faces_count));
    {
