...
use vector_traits::{
    ...
    prelude::{Aabb2, Aabb3, Affine3D, GenericVector2, GenericVector3, HasXY, HasXYZ, Plane},
};
    ...
    FFIVector3: ConvertTo<T>,
{
    ...
    if plane != Plane::XY {
        return Err(HallrError::InvalidInputData(format!(
            "At the moment the cmd_voronoi_diagram mesh operation only supports input data in the XY plane. {plane:?}",
        )));
    }
