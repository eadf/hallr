...
class HALLR_PT_BabySharkTool(bpy.types.Panel):
    """A CNC toolpath/mesh probe operation, work in progress"""
    bl_label = "Mesh Simplification"
    bl_idname = "HALLR_PT_bs_mesh_simplification"
    ...
    def draw(self, context):
        layout = self.layout
        settings = context.scene.hallr_baby_shark_settings

        row = layout.row(align=True)
        # 3D mesh for height offsets
        if settings.mesh is not None:
            row.operator("object.hallr_mt_bs_select_mesh", text="De-Select mesh", icon='X')
        else:
            row.operator("object.hallr_mt_bs_select_mesh", text="Select mesh", icon='EYEDROPPER')
        if settings.mesh is not None:
        ...
        if settings.mesh is not None:
            layout.row(align=True).operator("object.hallr_bs_process_mesh", text="Simplify")

class OBJECT_OT_MTSelectMesh(bpy.types.Operator):
    """ Select the object that contains the mesh """
    bl_idname = "object.hallr_mt_bs_select_mesh"
...
class OBJECT_OT_BS_ProcessMesh(bpy.types.Operator):
    """ Execute the mesh simplification"""
    bl_idname = "object.hallr_bs_process_mesh"
    bl_label = "Mesh Simplification (decimation)"
...
class BabyShaekToolSettings(bpy.types.PropertyGroup):
...
classes = (
    BabyShaekToolSettings,
    HALLR_PT_BabySharkTool,
    OBJECT_OT_MTSelectMesh,
    OBJECT_OT_BS_ProcessMesh,
)
...
def register():
    ...
    bpy.types.Scene.hallr_baby_shark_settings = bpy.props.PointerProperty(type=BabyShaekToolSettings)
...
def unregister():
    for cls in reversed(classes):
        try:
            ...
            pass
    try:
        del bpy.types.Scene.hallr_baby_shark_settings
